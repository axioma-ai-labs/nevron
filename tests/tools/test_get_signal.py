...
@pytest.fixture
def coinstats_tool():
    """Fixture to create a CoinstatsTool instance."""
    return CoinstatsTool()


@pytest.mark.skip(reason="Something is wrong with mocking the Coinstats API")
@pytest.mark.asyncio
async def test_get_coinstats_news_success(mock_tool_logger, mock_httpx_client, coinstats_tool):
    ...
    with patch("httpx.AsyncClient", return_value=mock_httpx_client):
        # act:
        result = await coinstats_tool.get_coinstats_news()
...
@pytest.mark.asyncio
async def test_get_coinstats_news_http_error(mock_tool_logger, mock_httpx_client, coinstats_tool):
    ...
    with (
        ...
        # act:
        await coinstats_tool.get_coinstats_news()
...
@pytest.mark.asyncio
async def test_fetch_signal_success(mock_tool_logger, coinstats_tool):
    """Test successful signal fetching."""
    # arrange:
    mock_debug, mock_error = mock_tool_logger
    news_data = {"result": [{"title": "Bitcoin reaches new high"}]}

    with patch.object(coinstats_tool, "get_coinstats_news", return_value=news_data):
        # act:
        result = await coinstats_tool.fetch_signal()
...
@pytest.mark.asyncio
async def test_fetch_signal_no_news(mock_tool_logger, coinstats_tool):
    ...
    with patch.object(coinstats_tool, "get_coinstats_news", new=mock_get_news):
        # act:
        result = await coinstats_tool.fetch_signal()
...
@pytest.mark.asyncio
async def test_fetch_signal_no_title(mock_tool_logger, coinstats_tool):
...
@pytest.mark.asyncio
async def test_fetch_signal_api_error(mock_tool_logger, coinstats_tool):
    """Test signal fetching when API call fails."""
    # arrange:
    mock_debug, mock_error = mock_tool_logger

    with patch.object(
        coinstats_tool, "get_coinstats_news", side_effect=CoinstatsError("Test error")
    ):
        # act:
        result = await coinstats_tool.fetch_signal()
